...
*/
Laser::Laser(double wavelength, QString name) : laser_wavelength(wavelength), laser_name(std::move(name)) {}

/*
Comparison operators:
*/
bool Laser::operator<(const Laser& other) const { return this->laser_wavelength < other.laser_wavelength; }
bool Laser::operator>(const Laser& other) const { return this->laser_wavelength > other.laser_wavelength; }
bool Laser::operator<=(const Laser& other) const { return this->laser_wavelength <= other.laser_wavelength; }
bool Laser::operator>=(const Laser& other) const { return this->laser_wavelength >= other.laser_wavelength; }
bool Laser::operator==(const Laser& other) const { return this->laser_wavelength == other.laser_wavelength; }
...
*/
const QString& Laser::name() const { return this->laser_name; }
...
Filter::Filter(Filter::Type type, double wavelength, double fwhm, QString name)
    : filter_type(type), filter_wavelength(wavelength), filter_fwhm(fwhm), filter_name(name) {}

/*
Getter: returns the type of the filter
*/
Filter::Type Filter::type() const { return this->filter_type; }

/*
Getter: returns the name of the filter
*/
const QString& Filter::name() const { return this->filter_name; }

/*
Getter: returns the central/edge wavelength of the filter
*/
double Filter::wavelength() const { return this->filter_wavelength; }

/*
Getter: returns the full width half maximum (if relevant, otherwise 0.0)
*/
double Filter::fwhm() const { return this->filter_fwhm; }
...
*/
LaserLine::LaserLine() : line_lasers(), line_filters() {
...
*/
LaserLine::LaserLine(std::size_t reserve_lasers, std::size_t reserve_filters) : line_lasers(), line_filters() {
...
*/
std::vector<Laser>& LaserLine::lasers() { return this->line_lasers; }

/*
Getter/Setter: returns a non-const reference to the Laser storage vector
*/
const std::vector<Laser>& LaserLine::lasers() const { return this->line_lasers; }

/*
Getter/Setter: returns a non-const reference to the Filter storage vector
*/
std::vector<Filter>& LaserLine::filters() { return this->line_filters; }

/*
Getter/Setter: returns a non-const reference to the Filter storage vector
*/
const std::vector<Filter>& LaserLine::filters() const { return this->line_filters; }
...
void LaserLine::sort() {
  // Sort the laser
  std::sort(this->line_lasers.begin(), this->line_lasers.end());
  // Sort the filters
  std::sort(this->line_filters.begin(), this->line_filters.end(),
...
*/
Instrument::Instrument() : instrument_id(QString()), instrument_name(QString()), instrument_optics() {}

/*
Constructor: Container of a optical instrument
*/
Instrument::Instrument(QString id, QString name, std::size_t laserline_count)
    : instrument_id(std::move(id)), instrument_name(std::move(name)), instrument_optics() {
...
*/
const QString& Instrument::id() const { return this->instrument_id; }

/*
Getter: returns the name of the instrument
*/
const QString& Instrument::name() const { return this->instrument_name; }

/*
Setter: returns a reference to modify the internal optics vector
*/
std::vector<LaserLine>& Instrument::setOptics() { return this->instrument_optics; }

/*
Getter: returns the optics of the instrument
*/
const std::vector<LaserLine>& Instrument::optics() const { return this->instrument_optics; }
...
void Instrument::sort() {
  ...
  std::sort(this->instrument_optics.begin(), this->instrument_optics.end(),
...
std::pair<const Data::LaserLine*, const Data::Laser*> Instrument::findLaser(double wavelength) const {
  ...
  for (std::size_t i = 0; i < this->instrument_optics.size(); ++i) {
    auto iterator = std::find_if(this->instrument_optics[i].lasers().cbegin(), this->instrument_optics[i].lasers().cend(),
...
*/
InstrumentReader::InstrumentReader() : instrument_data(), instrument_ids() {}
...
void InstrumentReader::load(const Data::Factory& factory) {
  ...
  std::sort(this->instrument_ids.begin(), this->instrument_ids.end(),
...
Instrument InstrumentReader::getInstrument(const QString& id) const {
  ...
  for (int i = 0; i < laserlines.size(); ++i) {
    ...
    optics.push_back(LaserLine(static_cast<std::size_t>(data_lasers.size()), static_cast<std::size_t>(data_filters.size())));
    ...
    for (int j = 0; j < data_filters.size(); ++j) {
      ...
      if (type == "BP") {
        optics[static_cast<std::size_t>(i)].filters().push_back(Filter(Filter::BandPass, data_filter["wavelength"].toDouble(0.0),
                                                                       data_filter["fwhm"].toDouble(0.0),
                                                                       data_filter["name"].toString("")));
      } else if (type == "LP") {
        optics[static_cast<std::size_t>(i)].filters().push_back(
            Filter(Filter::LongPass, data_filter["wavelength"].toDouble(0.0), 0.0, data_filter["name"].toString("")));
      } else if (type == "SP") {
        optics[static_cast<std::size_t>(i)].filters().push_back(
            Filter(Filter::ShortPass, data_filter["wavelength"].toDouble(0.0), 0.0, data_filter["name"].toString("")));
      }
    }
  }
...
/*
Getter for the list of instrument id's
*/
const std::vector<InstrumentID>& InstrumentReader::getInstruments() const { return this->instrument_ids; }

/*
Unallocates memory of all loaded data
*/
void InstrumentReader::unload() { this->instrument_data = QJsonDocument(); }

/*
Returns whether Data::InstrumentReader is valid (has loaded cytometer data)
  :returns: validity
*/
bool InstrumentReader::isValid() const { return !this->instrument_data.isNull(); }

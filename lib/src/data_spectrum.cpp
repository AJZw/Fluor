...
*/
Meta::Meta() : excitation_max(-1), emission_max(-1) {}
...
*/
Meta::Meta(double excitation_max, double emission_max) : excitation_max(excitation_max), emission_max(emission_max) {}
...
*/
Polygon::Polygon() : x_min(-1.0), x_max(-1.0), y_min(-1.0), y_max(-1.0), curve_color(0, 0, 0), curve(2) {
...
Polygon::Polygon(double x_min, double x_max, double y_min, double y_max, QPolygonF curve)
    : x_min(x_min), x_max(x_max), y_min(y_min), y_max(y_max), curve_color(0, 0, 0), curve(curve) {}

/*
Copy constructor: copy construct a polygon object.
Necessary cause the internal QVector<QPointF> of QPolygonF defaults to shallow copies. As polygon objects are copied for plotting they will
always be modified. I would like to incur this performance hit upon curve adding and upon the first draw.
*/
Polygon::Polygon(const Polygon& other)
    : x_min(other.x_min), x_max(other.x_max), y_min(other.y_min), y_max(other.y_max), curve_color(other.curve_color), curve(other.curve) {
...
//   }
//
//   return stream << "Data::Polygon{" << object.x_min << "-" << object.x_max << ":" << object.curve[0] << "-" <<
//   object.curve[object.curve.size() - 1] << "}";
// }
...
/*
Returns the intensity at the specified wavelength. Calculates the index to it and returns that wavelength.
Assumes a linear distribution of the curve to be able to calculate the index
  ...
  :param cutoff: if intensity is below cutoff, reduces intensity to 0.0
...
double Polygon::intensityAt(double wavelength, double cutoff) const {
  ...
  // Prevent indexing outside of curve; can happen if fluorophore.json data does not adhere to 1nm steps for x.
  if (index < this->curve.size()) {
    intensity = this->curve[index].y();
  }
...
double Polygon::intensityAtIter(double wavelength, double cutoff) const {
  ...
  double fraction = (wavelength - x_min) / (x_max - x_min);
  auto index =
      std::lower_bound(this->curve.cbegin(), this->curve.cend(), fraction, [](const QPointF& lhs, double rhs) { return lhs.x() < rhs; });
...
/*
Finds the wavelength of the (first) highest y value. Technically could look for value >=1.0, but quite some uncleaned datasets dont reach
that value. :returns: wavelength in nanometers
*/
...
Getter for the line color
  :returns: line color
*/
const QColor& Polygon::color() const { return this->curve_color; }
...
*/
void Polygon::setColor(double wavelength) { this->curve_color = Polygon::visibleSpectrum(wavelength); }

/*
Setter for the line color. Sets supplied QColor
  :param color: line color
*/
void Polygon::setColor(QColor color) { this->curve_color = color; }

/*
Getter for the curve
  :returns: the curve polygon
*/
QPolygonF& Polygon::polygon() { return this->curve; }
...
bool Polygon::contains(const QPointF& point, double line_width) const {
  ...
  auto entree_y = std::lower_bound(this->curve.begin(), this->curve.end(), point.x(),
                                   [](const QPointF& obj_a, double obj_b) { return obj_a.x() < obj_b; });
...
*/
void Polygon::scale(const Data::Polygon& base, const QRectF& size, const double xg_begin, const double xg_end, const double yg_begin,
                    const double yg_end, const double intensity) {
  ...
  if (base.curve.capacity() != this->curve.capacity()) {
    qWarning() << "Data::Polygon::scale: base and this object have different capacity QPolygonF, operation is not guaranteed to be safe, "
...
Scales the curve in the given/local space according to the global space
  ...
  :param intensity: the y (intensity) scaling value
*/
void Polygon::scale(const Data::Polygon& base, const QRectF& size, std::function<double(double)> scale_x,
                    std::function<double(double, double)> scale_y, const double intensity) {
  // Double check for base size, there should not be any resizing of the internal QVector
  if (base.curve.capacity() != this->curve.capacity()) {
    qWarning() << "Data::Polygon::scale: base and this object have different capacity QPolygonF, operation is not guaranteed to be safe, "
                  "function call is ignored.";
...
Spectrum::Spectrum(QString id)
    : absorption(false), two_photon(false), fluor_id(id), polygon_excitation(Polygon()), polygon_emission(Polygon()) {}
...
Spectrum::Spectrum(QString id, Polygon excitation, Polygon emission)
    : absorption(false), two_photon(false), fluor_id(id), polygon_excitation(excitation), polygon_emission(emission) {}

/*
Getter - returns the fluorophore id of the Spectrum object
*/
QString Spectrum::id() const { return (this->fluor_id); }
...
*/
const Data::Polygon& Spectrum::excitation() const { return this->polygon_excitation; }

/*
Getter for the emission polygon
  :returns: emission polygon
*/
const Data::Polygon& Spectrum::emission() const { return this->polygon_emission; }

/*
Setter for the excitation data,
  :param polygon: the excitation curve
*/
void Spectrum::setExcitation(Polygon polygon) { this->polygon_excitation = std::move(polygon); }

/*
Setter for the emission data
  :param polygon: the emission curve
*/
void Spectrum::setEmission(Polygon polygon) { this->polygon_emission = std::move(polygon); }

/*
Getter for the absorption flag
  :returns: is the excitation curve actually an absorption curve
*/
bool Spectrum::absorptionFlag() const { return this->absorption; }

/*
Getter for the two photon flag
  :returns: is the emission curve actually an two photon curve
*/
bool Spectrum::twoPhotonFlag() const { return this->two_photon; }
...
*/
void Spectrum::setAbsorptionFlag(bool flag) { this->absorption = flag; }

/*
Setter for the two_photon flag; to-be used when instead of emission a two_photon polygon is used
  :param flag: whether or not absorption is used
*/
void Spectrum::setTwoPhotonFlag(bool flag) { this->two_photon = flag; }

/*
Returns the excitation intensity at a specific wavelength
  :param wavelength: the excitation wavelength
  :returns: the excitation intensity fraction, out of bounds returns 0.0, an unspecified wavelength returns the first specified lower
value
*/
double Spectrum::excitationAt(double wavelength, double cutoff) const { return this->polygon_excitation.intensityAt(wavelength, cutoff); }
...
*/
double Spectrum::emissionAt(double wavelength, double cutoff) const { return this->polygon_emission.intensityAt(wavelength, cutoff); }

/*
Returns the wavelength of the maximum excitation intensity
  :returns: the wavelength of the (first) maximum excitation
*/
double Spectrum::excitationMax() const { return this->polygon_excitation.intensityMax(); }

/*
Returns the wavelength of the maximum emission intensity
  :returns: the wavelength of the (first) maximum emission
*/
double Spectrum::emissionMax() const { return this->polygon_emission.intensityMax(); }
...
Getter for the build index
  :returns: build index
*/
unsigned int CacheSpectrum::index() const { return (this->cache_index); }

/*
Setter for the build index
  :param: index
*/
void CacheSpectrum::setIndex(unsigned int index) { this->cache_index = index; }

/*
Getter for the fluorophore id
  :returns: fluorophore id
*/
QString CacheSpectrum::id() const { return (this->spectrum_data.id()); }
...
*/
bool CacheSpectrum::selectExcitation() const { return this->select_excitation; }

/*
Getter for the select parameter of the emission plot
  :returns: emission select
*/
bool CacheSpectrum::selectEmission() const { return this->select_emission; }
...
*/
double CacheSpectrum::intensityCutoff() const { return (this->intensity_cutoff); }

/*
Setter for the intensity cutoff
  :params: intensity cutoff
*/
void CacheSpectrum::setIntensityCutoff(const double cutoff) { this->intensity_cutoff = cutoff; }

/*
Resets the modified flag to unmodified
*/
void CacheSpectrum::resetModified() { this->modified = false; }

/*
Getter for the internal spectrum object
  :returns: spectrum reference
*/
const Data::Spectrum& CacheSpectrum::spectrum() const { return this->spectrum_data; }

/*
Getter for the internal spectrum object.
  :returns: a copy of the spectrum
*/
Data::Spectrum CacheSpectrum::copySpectrum() const { return this->spectrum_data; }

/*
Getter for the polygon absorption flag
  :returns: absorption flag
*/
bool CacheSpectrum::absorptionFlag() const { return this->spectrum_data.absorptionFlag(); }

/*
Getter for the polygon two photon flag
  :returns: two photon flag
*/
bool CacheSpectrum::twoPhotonFlag() const { return this->spectrum_data.twoPhotonFlag(); }
...
*/
double CacheSpectrum::excitationAt(double wavelength) const { return this->spectrum_data.excitationAt(wavelength, 0.0); }
/*
Returns the emission intensity (0.0 - 100.0) at the specified wavelength
  :param wavelength: the wavelength to find the intensity of
*/
double CacheSpectrum::emissionAt(double wavelength) const { return this->spectrum_data.emissionAt(wavelength, 0.0); }

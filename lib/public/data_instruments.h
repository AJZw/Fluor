...
class DATALIB_EXPORT LaserLine {
 ...
 public:
  friend QDebug operator<<(QDebug stream, const LaserLine& object) {
    return stream << "{laser(" << object.line_lasers.size() << "):filters(" << object.line_filters.size() << ")}";
  };
...
struct DATALIB_EXPORT LaserID {
  LaserID(const Data::Laser* laser, const Data::LaserLine* laserline = nullptr)
      : laser(laser), laserline(laserline), custom_wavelength(0.0){};
...
class DATALIB_EXPORT Instrument {
 ...
 public:
  friend QDebug operator<<(QDebug stream, const Instrument& object) {
    return stream << "{" << object.instrument_id << ":laserlines(" << object.instrument_optics.size() << ")}";
  };

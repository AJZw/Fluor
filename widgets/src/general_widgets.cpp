...
Constructor:
*/
StyledWidget::StyledWidget(QWidget* parent) : QWidget(parent), window_screen(nullptr) {}
...
*/
QString StyledWidget::layoutMargins() const { return QString::number(this->contentsMargins().left(), 'f', 0); }
...
Getter for layout spacing property, as it has to return a QString it returns the value in pixels
*/
QString StyledWidget::layoutSpacing() const { return QString::number(this->layout()->spacing(), 'f', 0); }
...
*/
AboutIcon::AboutIcon(QWidget* parent) : QLabel(parent) {
...
*/
QString AboutIcon::iconScale() const { return QString::number(this->pixmap()->width(), 'f', 0); }
...
*/
AboutWindow::AboutWindow(QWidget* parent) : StyledWidget(parent) {
  ...
  QLabel* label_copyright = new QLabel(" Copyright 2023 - AJ Zwijnenburg - all rights reserved", this);
  QLabel* label_qt = new QLabel(
      " This program uses the Qt library under the GNU Lesser General\n Public Library version 3. You can find a copy of this license at\n "
...
Constructor for a separator (---) item for QListViews
  :param parent: parent widget
*/
Separator::Separator(QWidget* parent) : QWidget(parent), pen("#FFFFFF"), height(6) { this->setVisible(false); }

/*
Getter and setter for the separator height
*/
void Separator::setSeparatorHeight(int height) { this->height = height; }
int Separator::separatorHeight() const { return this->height; }

/*
Getter and setter for the separator line color
*/
void Separator::setSeparatorColor(QString color) { this->pen.setColor(QColor(color)); }
QString Separator::separatorColor() const { return this->pen.color().name(); }

/*
Getter and setter for the separator line width
*/
void Separator::setSeparatorWidth(int width) { this->pen.setWidth(width); }
int Separator::separatorWidth() const { return this->pen.width(); }
...
*/
SeparatorItem::SeparatorItem(QWidget* parent) : QStandardItem(), style(parent) {

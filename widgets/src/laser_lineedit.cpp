...
*/
AbstractPopup::AbstractPopup(QWidget* widget) : QListView(widget), margin_scrollbar(3), max_size() {
...
*/
QString AbstractPopup::viewportMarginsScrollBar() const { return QString::number(this->margin_scrollbar, 'f', 0); }
...
*/
void AbstractPopup::hidingScrollBar() { this->setViewportMargins(0, 0, 0, 0); }

/*
Slot: Receives showing signal from the vertical scrollbar and adds scrollbar margin
*/
void AbstractPopup::showingScrollBar() { this->setViewportMargins(0, 0, this->margin_scrollbar, 0); }
...
*/
void AbstractPopup::reloadModel(const Data::Instrument& instrument) { this->buildModel(instrument); }
...
Constructor: Builds the popup for the Laser::LineEdit
  :param parent: parent widget
*/
LaserLinePopup::LaserLinePopup(QWidget* widget) : AbstractPopup(widget) {
...
std::vector<Data::LaserID> LaserLinePopup::active_items() const {
  ...
  for (int row = 0; row < this->model()->rowCount(); ++row) {
    ...
    if (index_row.data(General::CustomItemTypeRole).canConvert<int>() &&
        qvariant_cast<int>(index_row.data(General::CustomItemTypeRole) == General::SeparatorItemType)) {
      // separator -> ignore
      continue;
    }
  ...
  switch (event->type()) {
    case QEvent::MouseButtonPress: {
      ...
      if (event_mouse->button() == Qt::LeftButton) {
        ...
        QModelIndex index_current = this->indexAt(point_local);
        if (index_current.data(General::CustomItemTypeRole).canConvert<int>() &&
            qvariant_cast<int>(index_current.data(General::CustomItemTypeRole) == General::SeparatorItemType)) {
    ...
    case QEvent::MouseButtonRelease: {
      ...
      if (event_mouse->button() == Qt::LeftButton) {
        ...
        } else if (index_current.data(General::CustomItemTypeRole).canConvert<int>() &&
                   qvariant_cast<int>(index_current.data(General::CustomItemTypeRole) == General::SeparatorItemType)) {
          // Do nothing if clicked on separator
        } else {
          // To set the states
...
Constructor: Builds the popup for the Laser::LineEdit
  :param parent: parent widget
*/
Popup::Popup(QWidget* widget) : AbstractPopup(widget) {
  // Set default data
  this->buildModel();
}
...
*/
void LineEdit::receiveGraphState(std::vector<State::GraphState>& state) { emit this->updatePopupGraphState(state); }

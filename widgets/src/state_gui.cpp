...
*/
GraphState::GraphState() : graph_lasers(), graph_laserline(nullptr), selected(false), visible_lasers(false), visible_filters(false) {}
...
GraphState::GraphState(bool visible_lasers, bool visible_filters)
    : graph_lasers(), graph_laserline(nullptr), selected(false), visible_lasers(visible_lasers), visible_filters(visible_filters) {}
...
GraphState::GraphState(std::size_t size)
    : graph_lasers(), graph_laserline(nullptr), selected(false), visible_lasers(false), visible_filters(false) {
...
*/
bool GraphState::isSelected() const { return this->selected; }

/*
Sets the selection state
  :param select: the new selection state
*/
void GraphState::setSelected(bool select) { this->selected = select; }

/*
Returns the visibility/drawing state of the lasers
*/
bool GraphState::visibleLasers() const { return this->visible_lasers; }

/*
Sets the visibility/drawing state of the lasers
  :param visible: the state to change to
*/
void GraphState::setVisibleLasers(bool visible) { this->visible_lasers = visible; }

/*
Returns the visibility/drawing state of the filters
*/
bool GraphState::visibleFilters() const { return this->visible_filters; }

/*
Sets the visibility/drawing state of the lasers
  :param visible: the state to change to
*/
void GraphState::setVisibleFilters(bool visible) { this->visible_filters = visible; }

/*
Returns the laser vector.
*/
const std::vector<Data::Laser>& GraphState::lasers() const { return this->graph_lasers; }

/*
Returns a non-const laser vector, for modification
*/
std::vector<Data::Laser>& GraphState::setLasers() { return this->graph_lasers; }

/*
Returns the laserline the lasers belong to (if applicable, if not returns a nullptr).
*/
const Data::LaserLine* GraphState::laserLine() const { return this->graph_laserline; }

/*
Sets the LaserLine pointer (is allowed to be nullptr)
*/
void GraphState::setLaserLine(const Data::LaserLine* laserline) { this->graph_laserline = laserline; }
...
*/
GUI::GUI() { this->graphs_state.reserve(this->graph_count_max); }

/*
Returns the state of the graphs.
(Allows for manipulation to allow graphs themself to manipulate selection states)
*/
std::vector<GraphState>& GUI::graphs() { return this->graphs_state; }

/*
Returns the number of graphs
*/
std::size_t GUI::graphCount() const { return this->graphs_state.size(); }
...
void GUI::removeGraph() {
  ...
  // Remove graph
  this->graphs_state.erase(std::next(this->graphs_state.begin(), i_diff), std::next(this->graphs_state.begin(), i_diff + 1));
}

/*
Resets the graph state to default by removing all graphs
*/
void GUI::clearGraphs() { this->graphs_state.clear(); }

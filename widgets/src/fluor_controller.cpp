...
*/
void Controller::setLayoutSpacing(QString layout_spacing) { this->layout()->setSpacing(layout_spacing.toInt()); }
...
*/
void Controller::receiveCacheAdd(std::vector<Data::FluorophoreID>& output) { emit this->sendCacheAdd(output); }

/*
Slot: receives and sends output from the scroll/button controllers and forwards towards cache
*/
void Controller::receiveCacheRemove(std::vector<Data::FluorophoreID>& output) { emit this->sendCacheRemove(output); }
...
*/
ScrollController::ScrollController(QWidget* parent) : QScrollArea(parent), button_widgets(), margin_scrollbar(0) {
...
*/
void ScrollController::receiveRemove(std::vector<Data::FluorophoreID>& fluorophores) { emit this->sendRemove(fluorophores); }
...
ButtonsController::ButtonsController(QWidget* parent)
    : QWidget(parent), widget_emission(nullptr), widget_excitation(nullptr), widget_remove(nullptr), id(""), name(""), data(nullptr) {
  this->setContentsMargins(0, 0, 0, 0);
...
Getter for layout spacing property, as it has to return a QString it returns the value in pixels
*/
QString ButtonsController::layoutSpacing() const { return QString::number(this->layout()->spacing(), 'f', 0); }

/*
Receives layout scaling properties from the stylesheet
*/
void ButtonsController::setLayoutSpacing(QString layout_spacing) { this->layout()->setSpacing(layout_spacing.toInt()); }
...
*/
void ButtonsController::receiveSelected() { emit this->sendSelected(this); }

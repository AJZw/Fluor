...
{
    setOrganizationName("AJZw");
    setApplicationName("Spectral Viewer");
    setApplicationVersion("v0.9.0");

    // Set icon
    if(QSysInfo().productType() == "osx"){
        setWindowIcon(QIcon(":/icons/viewer_black.icns"));
        ...
        if(5 * windows_green + 2 * windows_red + windows_blue >= 1024){
            setWindowIcon(QIcon(":/icons/viewer_black.ico"));
        }else{
            setWindowIcon(QIcon(":/icons/viewer_white.ico"));
        }
...
/*
Set and load application stylesheet
*/
...
{
    window_title = "Spectral Viewer";
    window_width = 800; // load from settings
    window_height = 300; // load from settings
